...
const DB_Storage = {
    ...
    initialize: function() {
        ...
        return self.getUserByAgentNumber(DB_CONFIG.ADMIN.agentNumber)
            .then(function(admin) {
                if (!admin) {
                    // First time ever — seed the admin account into Firebase
                    return self.addUser({
            ...
            .catch(function(err) {
                console.error('[Delta Black] Initialization error:', err);
                // Surface a readable message if the DB URL is wrong
                if (err.code === 'PERMISSION_DENIED') {
                    alert('Firebase permission denied. Open Firebase console → Realtime Database → Rules and set read/write to true for development.');
                } else if (err.message && err.message.includes('databaseURL')) {
                    alert('Firebase Realtime Database URL not found. Go to Firebase console → Realtime Database → Create database, then copy the URL.');
                }
            });
    },
