const DB_User = {
    showDashboard: function(user) {
        ...
        DB_Storage.getWelcomeMessage().then(function(welcomeMsg) {
            ...
            self.updateMenu(user);
            
            // Start real-time listener for new questions
            self.startQuestionListener(user.agentNumber);
        });
    },

    startQuestionListener: function(agentNumber) {
        var self = this;
        DB_Storage.listenForQuestions(agentNumber, function(questions) {
            console.log('Real-time questions update:', questions.length);
    ...
    loadQuestions: function(user) {
        ...
        DB_Storage.getQuestionsForAgent(user.agentNumber).then(function(questions) {
            return DB_Storage.getAnsweredQuestionIds(user.agentNumber).then(function(answeredIds) {
                var pendingQuestions = questions.filter(function(q) { 
                    return !answeredIds.includes(q.id); 
                });
                ...
                if (currentQ.type === 'text') {
                    ...
                    contentHTML = '<img src="' + currentQ.content + '" alt="Question Image" style="max-width:100%; border-radius:4px;">';
                } else if (currentQ.type === 'video') {
                    contentHTML = '<video controls src="' + currentQ.content + '" style="max-width:100%; border-radius:4px;"></video>';
